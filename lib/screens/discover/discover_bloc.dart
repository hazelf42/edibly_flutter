    ...
    class DiscoverBloc {
      ...
      void _fetchEvents() async {
        ...
        /// network request
            ...
            await http.get('http://edibly.vassi.li/api/events').then( (http.Response response) {
           final map = json.decode(response.body);
            map.forEach((event) =>
            eventsWithoutExtraData.add(Data(event['eid'], event)));  
        });
        ...
        for (var dataWithoutRating in eventsWithoutExtraData) {
          ...
          try {
            ...
            dataWithRating.value['lon'] = restaurant['lon']/1000000;  
            dataWithRating.value['distance'] = _distanceFromMeToDestination(LatLng(
              double.parse(dataWithRating.value['lat'].toString()),
              double.parse(dataWithRating.value['lon'].toString()),
            ));
