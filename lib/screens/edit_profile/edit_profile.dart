...
class EditProfile extends StatefulWidget {
  EditProfile(this._firebaseUser);

  final FirebaseUser _firebaseUser;

  @override
  _EditProfileState createState() => _EditProfileState(_firebaseUser);
}

class _EditProfileState extends State<EditProfile> {
  final FirebaseUser _firebaseUser;
  _EditProfileState(this._firebaseUser);
    ...
    return FutureBuilder<http.Response>(
      ...
      builder: (context, snapshot) {
        Map<dynamic, dynamic> map =
            snapshot.hasData ? json.decode(snapshot.data.body) : null;
        if (map != null) {
          mainBloc.setDiet(
              null,
              map['veglevel'].toString().toLowerCase() == '2'.toLowerCase()
                  ? Diet.VEGAN
                  : Diet.VEGETARIAN);
          ...
          child: Row(
            children: <Widget>[
              CircleAvatar(
                radius: 24.0,
                backgroundImage:
                    map == null ? null : NetworkImage(map['photo'] ?? ''),
                child: map == null
                    ? SizedBox(
                        width: 46.0,
                        height: 46.0,
          ...
          return Drawer(
            child: ListView(
              ...
              children: <Widget>[
                ...
                MenuItem(
                  onTap: () async {
                    final newPhotoUrl = await getImage();
                    await http.put("http://edibly.vassi.li/api/profiles/${_firebaseUser.uid}", body: json.encode( {
                      'photo' : "https://i.chzbgr.com/full/5718464256/h8A74BF92/",
                ...
                StreamBuilder<Diet>(
                  ...
                  builder: (context, snapshot) {
                    return MenuSelector(
                      ...
                      selectedOption: snapshot.data == Diet.VEGETARIAN
